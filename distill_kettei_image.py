...
if __name__ == "__main__":
    ...
    three_dim_model = networks.__dict__["TDModel_2d"](opts, 3, 32)
    ...
    print('=====================================================================')
    three_dim_model = utils.load_model(three_dim_model, "checkpoints/wGT_img_2d_full/derain_self/model_epoch_100.pth", opts, epoch_st)
    ...
    while three_dim_model.epoch < opts.epoch_max:
        ...
        for iteration, all_batch in enumerate(data_loader, 1):
            ...
            for st in range(0, frame_per_video - 7):
                ...
                # -------------- image forward
                frame_input = warp_i3.squeeze()#torch.cat((warp_i2.detach(), warp_i3.detach(), warp_i4.detach()), 2)
