...
class RandomCrop(object):
    ...
    def __call__(self, img):
        # print('hello', img.shape, self.h1, self.w1, self.h2, self.w2)
        if len(img.shape) == 3:
...
class MultiFramesDataset(data.Dataset):
    ...
    def __init__(self, opts, mode):
        ...
        for video in videos:
            ...
            gt_dir = os.path.join(self.opts.data_dir, self.mode, video[:-7]+"GT")
            frame_list = glob.glob(os.path.join(input_dir, '*.jpg'))
            gt_list = glob.glob(os.path.join(gt_dir, '*.jpg'))
    ...
    def __getitem__(self, index):
        ...
        for t in range(T+1, T + self.opts.sample_frames+1):
            gt_i.append(utils.read_img(os.path.join(gt_dir, "%05d.jpg" % t)))
        ...
        if self.mode == 'train':
            if self.opts.geometry_aug:
                ...
                for t in range(self.opts.sample_frames):
                    ...
                    gt_i[t] = cv2.resize(gt_i[t], (W_out, H_out))


            cropper = RandomCrop(frame_i[0].shape[:2], (self.opts.crop_size, self.opts.crop_size))

            for t in range(self.opts.sample_frames):
                frame_i[t] = frame_i[t][:self.opts.crop_size, :self.opts.crop_size]
                gt_i[t] = gt_i[t][:self.opts.crop_size, :self.opts.crop_size]

            
            # print(i, ": ", frame_i[0].shape)
            ...
            if self.opts.geometry_aug:
                ...
                if np.random.random() >= 0.5:
                    for t in range(self.opts.sample_frames):
                        ...
                        gt_i[t] = cv2.flip(gt_i[t], flipCode=0)
            ...
            for t in range(self.opts.sample_frames):
                ...
                gt_i[t] = cv2.resize(gt_i[t], (W_o, H_o))
        else:
        ...
        for t in range(self.opts.sample_frames):
            ...
            data_gt.append(torch.from_numpy(gt_i[t].transpose(2, 0, 1).astype(np.float32)).contiguous())
