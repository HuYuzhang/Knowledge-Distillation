...
def save_model5(three_dim_model, fusion_model, flow_model, optimizer, opts):
...
def save_model6(three_dim_model, lstm_model_t, lstm_model_s_front, lstm_model_s_back, optimizer, opts):
    ...
    state_dict = {
            'three_dim_model': three_dim_model.state_dict(),
            'lstm_model_t' : lstm_model_t.state_dict(),
            'lstm_model_s_front': lstm_model_s_front.state_dict(),
            'lstm_model_s_back': lstm_model_s_back.state_dict(),
            'optimizer' : optimizer.state_dict(),
        }
...
def save_model3(three_dim_model, optimizer, opts):
...
def save_model4(three_dim_model, lstm_model, optimizer, opts):
...
def load_model(three_dim_model, fusion_model, flow_model, optimizer, opts, epoch):
    ...
    return three_dim_model, fusion_model, flow_model, optimizer

def load_model(three_dim_model, path, opts, epoch):
...
def get_Gmean(x):
    # X of shape (bsz, C, H, W)
    x_chan_mean = torch.sum(x, dim=1)
    x_Gmean = torch.pow(x_chan_mean, 2)
    return x_Gmean

def save_np(img, filename):
